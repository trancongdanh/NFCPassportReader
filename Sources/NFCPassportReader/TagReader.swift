            ...
            if let error = error {
                ...
                if let sm = self.secureMessaging {
                    do {
                        ...
                        Log.verbose(String(format:"TagReader [SM - unprotected] \(Utils().binToHexRep(rep.data, asArray:true)), sw1:0x%02x sw2:0x%02x", rep.sw1, rep.sw2) )
                    } catch {
                        completed( nil, NFCPassportReaderError.UnableToUnprotectAPDU )
                        return
                    }
                } else {
                    Log.verbose(String(format:"TagReader [unprotected] \(Utils().binToHexRep(rep.data, asArray:true)), sw1:0x%02x sw2:0x%02x", rep.sw1, rep.sw2) )
