...
public class SecureMessaging {
    ...
    func protect(apdu : NFCISO7816APDU ) throws -> NFCISO7816APDU {
        ...
        Log.verbose("\t\tSSC: " + Utils().binToHexRep(self.ssc))
        ...
        Log.verbose("\t\tCC: " + Utils().binToHexRep(CC))
        ...
        Log.verbose("\tProtectedAPDU: " + Utils().binToHexRep(protectedAPDU))
    ...
    func unprotect(rapdu : ResponseAPDU ) throws -> ResponseAPDU {
        ...
        Log.verbose("\tRAPDU: " + Utils().binToHexRep(rapduBin))
        ...
        if rapduBin[offset] == 0x8E {
            ...
            Log.verbose("\t\tK: " + Utils().binToHexRep(K))
            ...
            Log.verbose("\t\tCC: " + Utils().binToHexRep(CCb))
            
            let res = (CC == CCb)
            Log.verbose("\tCompare CC with data of DO'8E of RAPDU")
            Log.verbose("\t\t\(Utils().binToHexRep(CC))  == \(Utils().binToHexRep(CCb)) ? \(res)")
        ...
        if do87Data.count > 0 {
            ...
            Log.verbose("\tDecryptedData: " + Utils().binToHexRep(data))
        }
        
        Log.verbose("Unprotected APDU: [\(Utils().binToHexRep(data))] \(binToHexRep(sw1)) \(binToHexRep(sw2))" )
    ...
    func maskClassAndPad(apdu : NFCISO7816APDU ) -> [UInt8] {
        ...
        Log.verbose("\tCmdHeader: " + Utils().binToHexRep(res))
        return res
    }
    ...
    func buildD087(apdu : NFCISO7816APDU) throws -> [UInt8] {
        ...
        Log.verbose("\tDO87: " + Utils().binToHexRep(res))
        return res
    }
    ...
    func padAndEncryptData(_ apdu : NFCISO7816APDU) -> [UInt8] {
        ...
        Log.verbose("\tData: " + Utils().binToHexRep(paddedData))
        Log.verbose("Encrypt data with KSenc")
        Log.verbose("\tEncryptedData: " + Utils().binToHexRep(enc))
        return enc
    }
    ...
    func buildD08E(mac : [UInt8]) -> [UInt8] {
        ...
        Log.verbose("\tDO8E: \(Utils().binToHexRep(res))" )
        return res
    }
